...
roll_pcr_bic_pls <- function(dat, target_col, pred_cols, h, cutoff_date, Kmax = 10) {
  ...
  yhat_rw0 <- rep(0, n)
  K_pick   <- rep(NA_integer_, n)   # <-- track K selected each decision
  ...
  for (i in i_start:n) {
    ...
    K_pick[i] <- best_K                     # <-- save K for this decision
  ...
  oos <- d %>%
    mutate(f_pcr = yhat_pcr,
           f_rw0 = yhat_rw0,
           K_selected = K_pick) %>%          # <-- expose K in OOS
    filter(!is.na(f_pcr))
  ...
  # quick K summary (optional)
  K_summary <- oos %>%
    count(K_selected, name = "n_decisions") %>%
    arrange(K_selected)
  ...
  summary <- tibble::tibble(
    ...
    K_avg = mean(oos$K_selected, na.rm = TRUE)          # <-- average K chosen
  )

  list(summary = summary, oos = oos, K_summary = K_summary)
}
```
