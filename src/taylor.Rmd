...
roll_ols_multi <- function(dat_h, h, cutoff_date){
  ...
  for (i in i_start:n){
    ...
    x_i <- dat_h[i - h, , drop = FALSE]
    yhat_tr[i] <- predict(fit, newdata = x_i)
...
fixed_split_ols <- function(dat, target_col, cutoff_date, h) {
  ...
  # IMPORTANT: labels Y_h(t)=log X_{t+h}-log X_t need t+h ≤ cutoff
